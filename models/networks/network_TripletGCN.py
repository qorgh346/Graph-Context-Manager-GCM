...
class TripletGCN(MessagePassing):  # Message Passing
    ...
    def forward(self, x, edge_feature, edge_index):
        ...
        gcn_x, gcn_e = self.propagate(edge_index, x=x, edge_feature=edge_feature)  # 메시지 패싱을 위한 초기 호출

         ### mlp 2단계 : 특정 노드의 경우 집합 단계에서 해당 노드(sub, obj)의 모든 가능한 연결에서 오는 신호들은 함께 평균화됨
        x = self.nn2(gcn_x) 
    ...
    def message(self, x_i, x_j, edge_feature):  # 노드에 대한 메시지 구성
        ...
        x = new_x_i + new_x_j #업데이트 된 source Node의 속성들(128개) + 업데이트 된 Target Node의 속성들(128개) = 요소합이라서
                                                                                            # (edge_index 개수 ,128)임
        # print('Message 패싱 끝 -> return 하면 aggregate 이웃 노드들의 특징 집계함')
        return [x, new_e]
...
if __name__ == '__main__':
    ...
    heads = 2
    x = torch.rand(4, 6) #node 4 , 6

    edge_feature = torch.rand([12, 40], dtype=torch.float)

    edge_index = torch.randint(0, 4, [12, 2])
    # print(edge_index.size())
    edge_index = edge_index.t().contiguous() #[2,12]
    # print(edge_index.size())
    #4,32
    net = TripletGCNModel(2, dim_node=6, dim_edge=40, dim_hidden=128)
